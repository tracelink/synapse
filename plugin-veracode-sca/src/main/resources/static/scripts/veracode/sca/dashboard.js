const barContentLoad = async () => {
  $('[data-toggle="tooltip"]').tooltip();
	...
	$('#projectFilter').change(updateSelection);
	$('#project').change(updateSelection);
...
const updateData = async () => {
    let response;
    if(viewBy == 'all'){
    	response = await fetch(`/veracode/sca/rest/issues?period=${period}&category=${category}`);
    } else {
      response = await fetch(`/veracode/sca/rest/issues?${viewBy}=${selection}&period=${period}&category=${category}`);
    }
	...
	toggleChart(hide, 'bar-no-data', 'issues-bar');
	...
	var ctx = document.getElementById('issues-bar').getContext('2d');

	// Generate the users by device chart.
	issuesBar = new Chart(ctx, {
		type: 'bar',
		data: data,
		options: options
	});
