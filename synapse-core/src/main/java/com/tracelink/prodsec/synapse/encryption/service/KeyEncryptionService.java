...
public class KeyEncryptionService {
	...
	private static final String LOAD_KEY_FAIL = "Could not load key from keystore. Please check that the keystore password and key alias are correct: ";
	private static final String UNKNOWN_ENC_TYPE = "Unknown encryption type: ";
	private static final String NULL_CIPHER_WRAP = "Cannot initialize cipher when trying to wrap data encryption key";
	private static final String CANNOT_WRAP = "Cannot wrap data encryption key";
	private static final String NULL_CIPHER_UNWRAP = "Cannot initialize cipher when trying to unwrap data encryption key";
	...
	public KeyEncryptionService(
			...
			@Value("${synapse.encryption.environment.currentKeyStorePath:#{null}}") String currentKeyStorePath,
			@Value("${synapse.encryption.environment.currentKeyStorePassword:#{null}}") String currentKeyStorePassword,
			@Value("${synapse.encryption.environment.currentKeyAlias:#{null}}") String currentKeyAlias,
			@Value("${synapse.encryption.environment.previousKeyStorePath:#{null}}") String previousKeyStorePath,
			@Value("${synapse.encryption.environment.previousKeyStorePassword:#{null}}") String previousKeyStorePassword,
			@Value("${synapse.encryption.environment.previousKeyAlias:#{null}}") String previousKeyAlias) {
	 ...
	 * @return the wrapped data encryption key, or null if Synapse is configured with {@link
	 * EncryptionType#NONE}.
	 */
	public String encryptKey(Key attribute) {
		// If attribute is null or Synapse is not configured to handle encryption, return null
	 ...
	 * to decrypt using the current key. This helps to ensure that Synapse can recover if the
	...
	public Key decryptKey(String dbData) {
		// If dbData is null or Synapse is not configured to handle encryption, return null
	 ...
	 *                being used
	 * @return the decrypted database data, or null if Synapse is configured with {@link
	 * EncryptionType#NONE}
	 */
