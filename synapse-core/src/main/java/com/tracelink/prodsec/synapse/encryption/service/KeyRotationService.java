...
import com.tracelink.prodsec.synapse.encryption.controller.EncryptionMgmtController;
import com.tracelink.prodsec.synapse.encryption.converter.AbstractEncryptedAttributeConverter;
import com.tracelink.prodsec.synapse.encryption.listener.EncryptionFlushEntityEventListener;
...
import com.tracelink.prodsec.synapse.encryption.repository.EncryptionMetadataRepository;
...
import com.tracelink.prodsec.synapse.scheduler.service.schedule.CronSchedule;
 ...
 * {@link DataEncryptionKeyRepository} to access the data encryption keys stored in the database
 * and the {@link KeyEncryptionService} to determine whether to rotate the key encryption key or to
...
public class KeyRotationService {
	...
	public KeyRotationService(
			@Value("${synapse.encryption.type:none}") EncryptionType encryptionType,
			@Value("${synapse.encryption.environment.decryptMode:#{false}}") boolean decryptMode,
		...
		switch (encryptionType) {
			case ENVIRONMENT:
				...
				if (decryptMode) {
					...
					// Generate new data encryption keys as needed and encrypt existing data for the first time
					encryptAddedColumns();
					// Resume scheduler service
					schedulerService.resume();
					// Set up auto-rotation schedule
					scheduleKeyRotationJob();
				}
				break;
			case NONE:
				break;
	 ...
	 * rotation in progress or is disabled, this method will skip rotation for that key. This
	 * method is called asynchronously from the {@link EncryptionMgmtController}, but only one key
	...
	/**
	 * Rotates the given {@link DataEncryptionKey}. There are three stages to a key rotation. In
	 * the first stage, a new key is generated to replace the current key, the {@code
	 * rotationInProgress} boolean is set to true and the {@code lastRotationDateTime} is updated.
	 * The data encryption key is then saved in the database to ensure recovery if Synapse goes
	 * down during the rotation. In the second stage, all entities that use the converter class
	 * associated with the given data encryption key are updated to re-encrypt the data with the
	 * new key. In the third stage, the {@code rotationInProgress} boolean is set to false and the
	 * data encryption key is saved again to complete the key rotation.
	 * <p>
	 ...
	 * be used to encrypt existing data for the very first time, or even to decrypt data that
	 * relies on the given data encryption key. The exact encryption and decryption logic for those
	...
	/**
	 * Updates all entities of the given entity class. Performs a {@link
	 * JpaRepository#findAll(Pageable)} and a {@link JpaRepository#saveAll(Iterable)} for each page
	 * of entities to re-encrypt the data with a new key. Pages contain up to 50 entities at a time.
	 *
	 ...
	 * rotations. This method is called if Synapse is configured with {@link
	 * EncryptionType#ENVIRONMENT}, but can also be called for {@link EncryptionType#NONE} if
	 * Synapse is configured to decrypt all data in the database.
	 */
	 ...
	 * Synapse is configured with {@link EncryptionType#ENVIRONMENT}.
	 */
	...
	/**
	 * Performs a rotation of the key encryption key by calling {@link
	 * DataEncryptionKeyRepository#findAll()} and a {@link DataEncryptionKeyRepository#saveAll(Iterable)}
	 * to re-encrypt the data encryption keys with the new key encryption key. This method is only
	 * called if Synapse is configured with {@link EncryptionType#ENVIRONMENT}.
	 */
	...
	/**
	 * Populates the annotations cache on startup for both {@link EncryptionType#NONE} and {@link
	 * EncryptionType#ENVIRONMENT}. The annotations cache is a static map from each entity class to
	 * the set of encrypted attribute converter class names. It improves performance during key
	 * rotations.
	 */
	 ...
	 * metadata. This method should only be invoked when Synapse is configured with {@link
	 * EncryptionType#ENVIRONMENT} and {@code decryptMode}.
	 */
	...
	private void scheduleKeyRotationJob() {
		schedulerService.scheduleKeyRotationJob(
	 ...
	 * only called if Synapse is configured with {@link EncryptionType#ENVIRONMENT} and {@code
	 * decryptMode} is true.
	 */
