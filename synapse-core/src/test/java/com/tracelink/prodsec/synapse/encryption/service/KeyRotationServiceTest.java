...
public class KeyRotationServiceTest {
	...
	public void testPostConstructEncryptionTypeNone() {
		KeyRotationService keyRotationService = new KeyRotationService(EncryptionType.NONE, false,
				...
				schedulerService, webApplicationContext, entityManagerFactory,
	...
	public void testPostConstructEncryptionTypeEnvironmentDecryptionConfigured() {
		KeyRotationService keyRotationService = new KeyRotationService(EncryptionType.ENVIRONMENT,
				...
				encryptionMetadataRepository, schedulerService, webApplicationContext,
				entityManagerFactory, encryptionFlushEntityEventListener);
		EventListenerGroup<FlushEntityEventListener> eventListenerGroup = configureEventListenerGroup();
		...
		Assert.assertEquals(1, annotationsCache.size());
		Assert.assertTrue(
	...
	public void testPostConstructWithRotation() {
		...
		Assert.assertEquals(dataEncryptionKey, captor.getValue().get(0));
	...
	public void testOnApplicationEvent() {
		...
		Assert.assertEquals(TestConverter.class.getName(),
				dataEncryptionKey.getConverterClassName());
		Assert.assertNotNull(dataEncryptionKey.getCurrentKey());
		Assert.assertEquals(LocalDate.now(),
				dataEncryptionKey.getLastRotationDateTime().toLocalDate());
		Assert.assertNull(dataEncryptionKey.getPreviousKey());
		Assert.assertFalse(dataEncryptionKey.isRotationInProgress());
	}
	...
	public void testGetKeys() {
		...
		Assert.assertEquals(dataEncryptionKey, keys.get(0));
	}
	...
	public void testRotateKeysInProgress() {
		...
		Assert.assertEquals("Skipping key rotation for converter " + dataEncryptionKey
				.getConverterClassName(), loggerRule.getMessages().get(0));
	}
	...
	public void testRotateKeys() {
		...
		Assert.assertEquals(testEntity, captor.getValue().get(0));

		Assert.assertEquals(key, dataEncryptionKey.getPreviousKey());
		Assert.assertEquals(LocalDate.now(),
				dataEncryptionKey.getLastRotationDateTime().toLocalDate());
		Assert.assertFalse(dataEncryptionKey.isRotationInProgress());
		Assert.assertFalse(loggerRule.getMessages().isEmpty());
		...
		Assert.assertEquals(
				"Finished data encryption key rotation for converter " + dataEncryptionKey
						.getConverterClassName(), loggerRule.getMessages().get(1));
	}
	...
	public void testRotateKeyInvalidId() {
		...
		Assert.assertFalse(loggerRule.getMessages().isEmpty());
		Assert.assertEquals("Skipping key rotation: no data encryption key found with the given id",
	...
	public void testRotateKey() {
		...
		Assert.assertEquals(2, captorLists.size());
		Assert.assertEquals(1, captorLists.get(0).size());
		Assert.assertEquals(testEntity1, captorLists.get(0).get(0));
		Assert.assertEquals(1, captorLists.get(1).size());
		Assert.assertEquals(testEntity2, captorLists.get(1).get(0));
	...
	public void testEnableRotationSchedule() {
		...
		Assert.assertFalse(encryptionMetadata.isRotationScheduleEnabled());
		...
		Assert.assertTrue(encryptionMetadata.isRotationScheduleEnabled());
		Assert.assertEquals(90, encryptionMetadata.getRotationPeriod(), 0.001);
	}
	...
	public void testEnableRotationScheduleDisable() {
		...
		BDDMockito.verify(encryptionMetadataRepository).saveAndFlush(encryptionMetadata);
		Assert.assertFalse(encryptionMetadata.isRotationScheduleEnabled());
		Assert.assertNull(encryptionMetadata.getRotationPeriod());
	}
	...
	public void testAutoRotateKeysNotEnabled() throws Exception {
		...
		Assert.assertEquals("Auto-rotation not configured", loggerRule.getMessages().get(0));
	...
	public void testAutoRotateKeys() throws Exception {
		...
		Assert.assertEquals("Starting data encryption key rotation for converter " + key3
				.getConverterClassName(), loggerRule.getMessages().get(0));
		Assert.assertEquals("Found entity class " + TestEntity.class.getName()
						+ " with @Convert annotation but cannot get associated repository.",
				loggerRule.getMessages().get(1));
		Assert.assertEquals("Finished data encryption key rotation for converter " + key3
				.getConverterClassName(), loggerRule.getMessages().get(2));
	...
	private static Method getMethod(String name, Class<?> target) {
		...
		if (!autoRotateMethod.isPresent()) {
			Assert.fail("Cannot find " + name + " method");
		}
