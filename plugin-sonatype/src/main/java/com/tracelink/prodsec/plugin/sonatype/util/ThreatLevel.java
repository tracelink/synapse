...
public enum ThreatLevel {
	HIGH("High", 8, 10, SonatypeMetrics::getHighVios, 25), MEDIUM("Medium", 4, 7,
			SonatypeMetrics::getMedVios, 5),
	LOW("Low", 2, 3, SonatypeMetrics::getLowVios, 2), INFO("Informational", 0, 1,
			SonatypeMetrics::getInfoVios, 0.5);
	...
	ThreatLevel(String name, int low, int high, ToLongFunction<SonatypeMetrics> viosCallback,
			double weight) {
		this.name = name;
		this.low = low;
		this.high = high;
	...
	/**
	 * Get the threat level for an integer value.
	 *
	 * @param level integer representation of threat level
	 * @return the threat level matching the given level
	 */
