...
const updateData = async () => {
	let response;
	if(viewBy == 'all'){
		response = await fetch(`/veracodedast/rest/reports?period=${period}&category=${category}`);    
    }else{
    	response = await fetch(`/veracodedast/rest/reports?${viewBy}=${selection}&period=${period}&category=${category}`);
    }
