CREATE TABLE veracode_dast_client_config (
...
CREATE TABLE veracode_dast_thresholds (
...
CREATE TABLE veracode_dast_apps (
  app_id BIGSERIAL NOT NULL,
  name varchar(255) DEFAULT NULL,
  synapse_project BIGINT DEFAULT NULL,
  PRIMARY KEY (app_id),
  CONSTRAINT fk_veracode_dast_synapse_project FOREIGN KEY (synapse_project) REFERENCES synapsecore.project (project_id)
);
...
CREATE TABLE veracode_dast_flaws (
  ...
  PRIMARY KEY (flaw_id),
  CONSTRAINT fk_flaws_report FOREIGN KEY (report) REFERENCES veracode_dast_reports (report_id)
);