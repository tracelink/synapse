...
const updateData = async () => {
    const response = await fetch(`/jira/rest/vulnmetrics?period=${period}`);
	...
	toggleChart(hide, 'bar-no-data', 'vuln-bar');
	...
	var ctx = document.getElementById('vuln-bar').getContext('2d');

	// Generate the users by device chart.
	vulnBar = new Chart(ctx, {
		type: 'bar',
		data: data,
		options: options
	});
